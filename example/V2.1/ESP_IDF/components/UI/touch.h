 ...
 * Enable exactly one controller and provide its board-specific pins.
 * Reference libraries:
...
// Provides event-based access to the FT6X36 controller.
FT6X36 ts(&Wire, TOUCH_FT6X36_INT);

// Preserve one pending press and release until LVGL consumes each event.
...
// Provides polled access to the GT911 controller.
 ...
 * @param e Touch event that describes the current gesture phase.
 * @return Nothing.
 ...
 *
 * Each controller branch configures its required I2C or SPI bus before setting
 * event handling or rotation.
 *
 * @param None
 * @return Nothing.
 ...
 *
 * Event-driven controllers update their pending flags here. The XPT2046 uses
 * its interrupt indication to avoid an SPI read when no touch is present.
 *
 * @param None
 * @return true when touch_touched() or touch_released() should be checked.
 * @return false when no input event is available.
 * @note The application or LVGL input callback calls this before reading data.
 */
...
/**
 * @brief Read a pressed state and update the calibrated pixel coordinates.
 *
 * Raw coordinates are mapped with controller-specific calibration endpoints.
 * Reversing an endpoint pair also reverses that axis when required by mounting.
 *
 * @param None
 ...
 *
 * The FT6X36 consumes a pending release flag. The polled controller branches
 * return true because their pressed state is determined by touch_touched().
 *
 * @param None
 * @return true when the pointer can be reported as released.
